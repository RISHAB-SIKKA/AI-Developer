        ...
        axios.get('/users/all')
          .then(res => {
            if (Array.isArray(res.data.users)) {
              ...
              console.warn("Unexpected users format:", res.data);
              setUsers([]);
            }
          })
          .catch(error => {
            ...
            <section className='left relative flex flex-col h-full min-w-96 bg-slate-300'>
                ...
                <div className='conversation-area flex-grow flex flex-col'>
                    ...
                    <div className='message-box p-2 flex-grow flex flex-col gap-2'>
                        <div className='message max-w-56 flex flex-col p-2 bg-slate-50 w-fit rounded-md'>
                            ...
                            <p className='text-sm'>Sample message from user</p>
                        </div>

                        <div className='ml-auto max-w-56 message flex flex-col p-2 bg-slate-50 w-fit rounded-md'>
                            <small className='opacity-65 text-sm'>example@gmail.com</small>
                            <p className='text-sm'>Sample message from self</p>
                        </div>
                    </div>
                ...
                <div className={`sidePanel w-full h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
                    ...
                    <div className='users flex flex-col gap-2'>
                        {/* Sample user (replace with actual project.users if needed) */}
